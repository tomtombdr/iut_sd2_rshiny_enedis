...
<h3>Contexte géographique et social</h3>
...
data <- df_filtered %>%
  ...
  # 1. Filtre les lignes où les coordonnées Lambert ne sont pas valides
...
# 2. Convertit et filtre les coordonnées WGS84 (Longitude/Latitude)
coords_filtrees <- coords %>%
  as.data.frame() %>%
  # Filtre basé sur une boîte englobante raisonnable pour les départements 73/74
  filter(
    X >= 5.5 & X <= 7.5,   # Longitude
    Y >= 45.0 & Y <= 46.5  # Latitude
  )
...
htmltools::div(
  ...
  leaflet() %>%
    addTiles() %>%
    addCircleMarkers(
      lng = coords_filtrees$X, # Utilisation des coordonnées filtrées
      lat = coords_filtrees$Y, # Utilisation des coordonnées filtrées
      radius = 3,
      color = "red",
      stroke = FALSE,
      fillOpacity = 0.6
...
<h3>Analyse des classes DPE sur les logements</h3>
